...
import { asuntoDe } from "@/lib/consultas";
import {
  listarCompras,
  listarConsultas,
  listarReservas,
 ...
 *
 * Cuenta sobre las reservas de hoy en adelante, las compras del último mes y
 * las consultas del chat que quedaron esperando respuesta. Son pocas: la granja
 * no toma más de un puñado por día, así que traerlas y contarlas acá es más
 * simple que pedirle un count por ficha a la base.
 */
export default async function ResumenPage() {
  ...
  let compras: Compra[];
  let derivadas: Consulta[];
  try {
    [proximas, compras, derivadas] = await Promise.all([
      listarReservas({ desde: hoy }),
      listarCompras({ desde: new Date(Date.now() - DIAS_DE_TIENDA * 86400000).toISOString() }),
      // las del chat que el asistente no supo contestar: esperan a una persona
      listarConsultas({ estado: "derivada" }),
    ]);
  } catch (error) {
  ...
  const fichas = [
    ...
    { valor: derivadas.length, etiqueta: "Consultas para contestar" },
  ];
  ...
  return (
    <>
      <header className="admin-head">
        ...
        <h1>Lo que viene</h1>
        <p>
          Lo que está tomado de hoy en adelante, lo que se vendió en la tienda este último mes y lo
          que quedó preguntado en el chat. El detalle, con teléfonos y comentarios, está en{" "}
          <Link href="/admin/reservas">Reservas</Link>, en <Link href="/admin/compras">Compras</Link>{" "}
          y en <Link href="/admin/consultas">Consultas</Link>.
        </p>
      </header>
      ...
      <section className="admin-bloque">
        <div className="admin-bloque-head">
          <h2>Consultas para contestar</h2>
          <Link href="/admin/consultas?estado=derivada">Ver el chat</Link>
        </div>

        {derivadas.length === 0 ? (
          <AdminAviso titulo="El asistente viene contestando todo">
            <p>Cuando le pregunten algo que no sabe, la consulta queda anotada acá.</p>
          </AdminAviso>
        ) : (
          <ul className="admin-proximas">
            {derivadas.slice(0, 8).map((consulta) => (
              <li key={consulta.id}>
                <span className="admin-proximas-fecha">{fechaHora(consulta.creada)} hs</span>
                <span className="admin-proximas-quien">
                  {asuntoDe(consulta.hilo)}
                  <em>
                    {consulta.codigo}
                    {consulta.contacto.telefono ? ` · ${consulta.contacto.telefono}` : ""}
                    {consulta.pagina ? ` · desde ${consulta.pagina}` : ""}
                  </em>
                </span>
                <span className={`admin-estado ${consulta.estado}`}>Para contestar</span>
              </li>
            ))}
          </ul>
        )}
